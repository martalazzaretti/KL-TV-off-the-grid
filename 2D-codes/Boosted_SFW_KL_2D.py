...
def Boosted_SFW_KULLBACK_2D(acquis, bg, sigma, X_domain, Y_domain, X_big, Y_big, ker='gaussian', m_0=0, par_reg=1e-5, nIter=5, BoostPar = float('inf'), verbose=False, plots=False, gt=0):
    ...
    def sliding_step(m):
        ...
        bnds_a = [(0, np.inf) for _ in range(N_k)]
        bnds_x = [(0, 1) for _ in range(2*N_k)]
