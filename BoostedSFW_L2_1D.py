...
def Boosted_SFW_L2_1D(X, acquis, bg, sigma, ker='gaussian', m_0=0, par_reg=1e-5, nIter=5, BoostPar = float('inf'), verbose=False, plots=False, gt=0):
    ...
    def frank_wolfe_step(m,x_new,max_eta):
        ...
        def lasso(a):
            aus = phi_vector(a, x_k, X, sigma, ker=ker) + bg
            fidelity = 0.5 * np.linalg.norm(acquis - aus, 2) ** 2
            penalty = par_reg * np.linalg.norm(a, 1)
            return (fidelity + penalty)
        def grad_lasso(a):
            grad = phi_vector(a, x_k, X, sigma, ker=ker) + bg
            grad = phi_amplitude_adjoint(grad - acquis, x_k, X, sigma, ker='gaussian') + par_reg * np.sign(a)
            return grad
    ...
    def sliding_step(m):
        ...
        def lasso_double(params):
            ...
            penalty = par_reg * np.linalg.norm(a, 1)
            return (fidelity + penalty)
        res = scipy.optimize.minimize(lasso_double, initial_guess, method='L-BFGS-B',
                                          options={'disp': True})
    ...
    x_star = np.argmax(np.abs(eta_V_k))/len(eta_V_k)
    eta_max = np.max(np.abs(eta_V_k))
