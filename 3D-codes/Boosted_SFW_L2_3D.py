...
def Boosted_SFW_L2_3D(y, bg, sig_x, sig_y, sig_z, X ,Y , Z,  N_xy, N_z, pixel_size_xy, pixel_size_z, par_reg=1e-5, nIter=5, m_0=0, BoostPar = float('inf'), verbose=False, plots=False, gt=0):
    ...
    def frank_wolfe_step(m,x_new,max_eta):
        ...
        def lasso(a):
            fidelity = 0.5*my3dnorm(y - phi_vector(a,x_k,X,Y,Z,sig_x, sig_y, sig_z))**2
    ...
    for k in range(nIter):
        ...
        x_k = measure_k.x
        Nk = measure_k.N
        energy_vector[k] = L2TV_cost_funct(measure_k, y, bg,  X, Y, Z, sig_x, sig_y, sig_z, par_reg)
        if verbose:
            print(f'* Energy : {energy_vector[k]:.3f}')
