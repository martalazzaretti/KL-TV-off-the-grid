...
def SFW_KULLBACK_3D(y, bg, sig_x, sig_y, sig_z, X ,Y , Z,  N_xy, N_z, pixel_size_xy, pixel_size_z, par_reg=1e-5, nIter=5, m_0=0):
...
#         plt.show()
        ...
        if np.abs(eta_V_k[x_star_index]) < 1:
            energy_k[k] = KLTV_cost_funct(measure_k, y, bg,  X, Y, Z, sig_x, sig_y, sig_z, par_reg)
            ...
            def grad_lasso_double(params):
                ...
                # Compute the gradient of the fidelity term
                aus = phi_vector(a_p,x_p,X,Y,Z,sig_x, sig_y, sig_z)
                residual = aus + bg - acquis + acquis * np.log(acquis) - acquis * np.log(aus + bg)
                for i in range(N):
            ...
            def lasso(a):
                aus = phi_vector(a,x_k_demi,X,Y,Z,sig_x, sig_y, sig_z)
                fidelity = np.sum(aus+bg-y+y*np.log(y) - y*np.log(aus+bg))
...
#             plt.show()
            ...
            bnds_x = [(-np.inf, np.inf) for _ in range(3*Nk+3)]
            bnds_new = bnds_a+bnds_x                               
            res = scipy.optimize.minimize(lasso_double, initial_guess,
                                          method='L-BFGS-B',
                                          bounds=bnds_new,
