...
def SFW_KULLBACK_1D(X, acquis, bg, sigma, ker='gaussian', m_0=0, par_reg=1e-5, 
                 nIter=5, sliding = 1, pruning = 1, verbose = False, plots = False, gt = 0):   
    ...
    for k in range(nIter):
        ...
        x_star_index = np.argmax(eta_V_k)
        ...
        if eta_max < 1:
            ...
            def lasso(a):
                aus = phi_vector(a,x_k_demi,X,sigma,ker=ker)
                fidelity = np.sum(aus+bg-acquis+acquis*np.log(acquis) - acquis*np.log(aus+bg))
                penalty = par_reg*np.linalg.norm(a, 1)
                return(fidelity + penalty)    
            def grad_lasso(a):
...
#             res = scipy.optimize.minimize(lasso, np.ones(Nk+1), jac=grad_lasso, bounds=bnds)
            ...
            if sliding == 1:
                ...
                bnds_x = [(0, 1) for _ in range(Nk+1)]
...
def Homotopy_SFW_KULLBACK_1D(X, acquis, bg, sigma, sigma_target, ker='gaussian', m_0=0,  nnIter=5, nIter=1, 
    ...
    # compute lambda_0 
    eta_max = np.max(etak_KL(measure_k, X, acquis, bg, 1, sigma, ker=ker))
    par_reg = 0.9*eta_max
    ...
    for k in range(nnIter): 
        ...
        (measure_k, nrj) = SFW_KULLBACK_1D(X, acquis, bg, sigma, ker=ker, m_0=measure_k, par_reg=par_reg, nIter=nIter, sliding = 1, pruning = 1, verbose=verbose, plots = plots, gt = gt)
        ...
        if sigma_t > sigma_target:
            eta_max = np.max(etak_KL(measure_k, X, acquis, bg, par_reg, sigma, ker=ker))
